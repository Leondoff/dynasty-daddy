...
@Component({
  ...
  export class WrappedFinishComponent implements OnInit {
    ...
    constructor(public configService: ConfigService, private leagueService: LeagueService, private playoffCalculatorService: PlayoffCalculatorService, public leagueSwitchService: LeagueSwitchService, public wrappedService: WrappedService) {}
    ...
    ngOnInit(): void {
      ...
      this.summaryCard = {first: seasonResults[0], second: seasonResults[1], bestOffense: teams[0], worstOffense: teams[teams.length-1], bestTrade: biggestTrade, standings: this.leagueService.leagueTeamDetails.slice(), totalWaivers: this.wrappedService.transactionsDict['waivers']?.length || 0, totalTrades: this.wrappedService.transactionsDict['trades']?.length || 0, leagueName: this.leagueService.selectedLeague.name};
      setInterval(()=> {
