...
Select *
From (
         WITH allTimeHighSFFc (name_id, fc_all_time_high_sf) as (select player_info.name_id    as name_id,
                                                                   max(pv.fc_sf_trade_value) as fc_all_time_high_sf
                                                            from player_info
                                                                     left join player_values pv on player_info.name_id = pv.name_id
                                                            group by player_info.name_id),
              allTimeHighFc (name_id, fc_all_time_high) as (select player_info.name_id as name_id,
                                                              max(pv.fc_trade_value) as fc_all_time_high
                                                       from player_info
                                                                left join player_values pv on player_info.name_id = pv.name_id
                                                       group by player_info.name_id),
              allTimeLowSFFc (name_id, fc_all_time_low_sf) as (select player_info.name_id    as name_id,
                                                                 min(pv.fc_sf_trade_value) as fc_all_time_low_sf
                                                          from player_info
                                                                   left join player_values pv on player_info.name_id = pv.name_id
                                                          group by player_info.name_id),
              allTimeLowFc (name_id, fc_all_time_low) as (select player_info.name_id as name_id,
                                                            min(pv.fc_trade_value) as fc_all_time_low
                                                     from player_info
                                                              left join player_values pv on player_info.name_id = pv.name_id
                                                     group by player_info.name_id),
              threeMonthHighSfFc (name_id, fc_three_month_high_sf) as (select player_info.name_id    as name_id,
                                                                         max(pv.fc_sf_trade_value) as fc_three_month_high_sf
                                                                  from player_info
                                                                           left join player_values pv on player_info.name_id = pv.name_id
                                                                  WHERE pv.created_at::date > now()::date - 90
         group by player_info.name_id),
    threeMonthHighFc (name_id, fc_three_month_high) as (select player_info.name_id as name_id,
    max(pv.fc_trade_value) as fc_three_month_high
from player_info
    left join player_values pv on player_info.name_id = pv.name_id
WHERE pv.created_at::date > now()::date - 90
group by player_info.name_id),
    threeMonthLowSfFc (name_id, fc_three_month_low_sf) as (select player_info.name_id    as name_id,
    min(pv.fc_sf_trade_value) as fc_three_month_low_sf
from player_info
    left join player_values pv on player_info.name_id = pv.name_id
WHERE pv.created_at::date > now()::date - 90
group by player_info.name_id),
    threeMonthLowFc (name_id, fc_three_month_low) as (select player_info.name_id as name_id,
    min(pv.fc_trade_value) as fc_three_month_low
from player_info
    left join player_values pv on player_info.name_id = pv.name_id
WHERE pv.created_at::date > now()::date - 90
group by player_info.name_id),
    lastMonthValue (name_id, fc_last_month_value_sf, fc_last_month_value) as (select player_info.name_id as name_id,
...
order by fc_sf_trade_value desc
WITH NO DATA;
...
CREATE MATERIALIZED VIEW if not exists mat_vw_dp_player_values
AS
Select *
From (
         WITH allTimeHighSFDp (name_id, dp_all_time_high_sf) as (select player_info.name_id    as name_id,
                                                                   max(pv.dp_sf_trade_value) as dp_all_time_high_sf
                                                            from player_info
                                                                     left join player_values pv on player_info.name_id = pv.name_id
                                                            group by player_info.name_id),
              allTimeHighDp (name_id, dp_all_time_high) as (select player_info.name_id as name_id,
                                                              max(pv.dp_trade_value) as dp_all_time_high
                                                       from player_info
                                                                left join player_values pv on player_info.name_id = pv.name_id
                                                       group by player_info.name_id),
              allTimeLowSFDp (name_id, dp_all_time_low_sf) as (select player_info.name_id    as name_id,
                                                                 min(pv.dp_sf_trade_value) as dp_all_time_low_sf
                                                          from player_info
                                                                   left join player_values pv on player_info.name_id = pv.name_id
                                                          group by player_info.name_id),
              allTimeLowDp (name_id, dp_all_time_low) as (select player_info.name_id as name_id,
                                                            min(pv.dp_trade_value) as dp_all_time_low
                                                     from player_info
                                                              left join player_values pv on player_info.name_id = pv.name_id
                                                     group by player_info.name_id),
              threeMonthHighSfDp (name_id, dp_three_month_high_sf) as (select player_info.name_id    as name_id,
                                                                         max(pv.dp_sf_trade_value) as dp_three_month_high_sf
                                                                  from player_info
                                                                           left join player_values pv on player_info.name_id = pv.name_id
                                                                  WHERE pv.created_at::date > now()::date - 90
         group by player_info.name_id),
    threeMonthHighDp (name_id, dp_three_month_high) as (select player_info.name_id as name_id,
    max(pv.dp_trade_value) as dp_three_month_high
from player_info
    left join player_values pv on player_info.name_id = pv.name_id
WHERE pv.created_at::date > now()::date - 90
group by player_info.name_id),
    threeMonthLowSfDp (name_id, dp_three_month_low_sf) as (select player_info.name_id    as name_id,
    min(pv.dp_sf_trade_value) as dp_three_month_low_sf
from player_info
    left join player_values pv on player_info.name_id = pv.name_id
WHERE pv.created_at::date > now()::date - 90
group by player_info.name_id),
    threeMonthLowDp (name_id, dp_three_month_low) as (select player_info.name_id as name_id,
    min(pv.dp_trade_value) as dp_three_month_low
from player_info
    left join player_values pv on player_info.name_id = pv.name_id
WHERE pv.created_at::date > now()::date - 90
group by player_info.name_id),
    lastMonthValue (name_id, dp_last_month_value_sf, dp_last_month_value) as (select player_info.name_id as name_id,
...
order by dp_sf_trade_value desc
WITH NO DATA;
